  ...
  for (unsigned int i = 0; i < textures.size(); i++)
    {
      ...
      //		qDebug() << QString(type + number).toLocal8Bit().constData() <<" : "<< i;
      program->setUniformValue(QString(type + number).toLocal8Bit().constData(), i);
      textures[i].texture->bind(static_cast<unsigned int>(i));
    }
  //	QVector3D light_pos(0.0f, 5.0f, -5.0f);
  //	QVector3D light_color(1.0f,1.0f,1.0f);
  //	QVector3D ambient_color=light_color*0.1f;
  //	QVector3D diffuse_color=light_color*0.9f;
  //	program->setUniformValue("light.position",light_pos);
  //	program->setUniformValue("light.ambient",ambient_color);
  //	program->setUniformValue("light.diffuse",diffuse_color);
  //	program->setUniformValue("light.specular",QVector3D(0.9f, 0.9f, 0.9f));
