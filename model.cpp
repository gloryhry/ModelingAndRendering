...
static QSharedPointer<QOpenGLTexture> textureFromFile(const QString &path, const QString &directory)
{
  ...
  qDebug()<<"texture:"<<fileName<<endl;
  QImage image(fileName);
  qDebug()<<"image size:"<<image.width()<<" "<<image.height()<<endl;
...
//  qDebug()<<"pscene:"<<path.toLocal8Bit().constData()<<endl;
  // 检查错误
...
//    }
//  qDebug()<< "pscene & ret: " << pscene << "|" << ret << endl;
//  return ret;
  processNode(pscene->mRootNode, pscene);
  return true;
}
...
{
  std::vector<Texture> textures;
  qDebug()<<"texture count:"<<material->GetTextureCount(type);
...
//      for(int i=0;i<len;i++)
//        {
//          qDebug()<<"str:"<<str->data[i];
//        }
      for(int i = 4;i<len;i++)
        {
          path.append(str->data[i]);
        }
