...
//uniform sampler3D texture_normal1;
...
//    result += CalcSpotLight(spotLight, norm, Pos, viewDir);
    vec3 result = CalcPointLight(pointLights[0],norm,Pos,normalize(viewPos - Pos));
    result += CalcPointLight(pointLights[1],norm,Pos,normalize(viewPos - Pos));
